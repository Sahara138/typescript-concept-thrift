...
const createConstraintWithGeneric = <T extends string | number | boolean>(value: T): T[] => {
    return [value];
}
const arrString = createConstraintWithGeneric('Hello');
const arrNumber = createConstraintWithGeneric(42);
const arrBoolean = createConstraintWithGeneric(true);
const arrObject = createArray   WithGeneric<{ id: number; name: string; age: number }>({ id: 1, name: 'John', age: 30 });
